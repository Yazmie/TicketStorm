...
import com.ticketstorm.service.delaysend.DelayOrderCancelSend;
import com.ticketstorm.service.kafka.CreateOrderMqDomain;
import com.ticketstorm.service.kafka.CreateOrderSend;
import com.ticketstorm.service.lua.ProgramCacheCreateOrderData;
import com.ticketstorm.service.lua.ProgramCacheCreateOrderResolutionOperate;
import com.ticketstorm.service.lua.ProgramCacheResolutionOperate;
import com.ticketstorm.service.tool.SeatMatch;
...
import static com.ticketstorm.service.constant.ProgramOrderConstant.ORDER_TABLE_COUNT;
...
public class ProgramOrderService {
        ...
        if (CollectionUtil.isNotEmpty(seatDtoList)) {
            for (SeatDto seatDto : seatDtoList) {
                ...
                if (Objects.nonNull(ticketCategoryVo)) {
                    ...
                    throw new TicketStormFrameException(BaseCode.TICKET_CATEGORY_NOT_EXIST_V2);
                }
            }
        } else {
        ...
        if (CollectionUtil.isNotEmpty(seatDtoList)) {
            ...
            for (Entry<Long, Long> entry : seatTicketCategoryDtoCount.entrySet()) {
                ...
                Long remainNumber = Optional.ofNullable(ticketCategoryRemainNumber.get(String.valueOf(ticketCategoryId)))
                        .orElseThrow(() -> new TicketStormFrameException(BaseCode.TICKET_CATEGORY_NOT_EXIST_V2));
                if (purchaseCount > remainNumber) {
                    throw new TicketStormFrameException(BaseCode.TICKET_REMAIN_NUMBER_NOT_SUFFICIENT);
                }
            }
            ...
            for (SeatDto seatDto : seatDtoList) {
                ...
                if (Objects.isNull(seatVo)) {
                    throw new TicketStormFrameException(BaseCode.SEAT_IS_NOT_NOT_SOLD);
                }
            ...
            if (parameterOrderPrice.compareTo(databaseOrderPrice) > 0) {
                throw new TicketStormFrameException(BaseCode.PRICE_ERROR);
            }
        }else {
            ...
            if (purchaseSeatList.size() < ticketCount) {
                throw new TicketStormFrameException(BaseCode.SEAT_OCCUPY);
            }
        }
    ...
    public List<SeatVo> createOrderOperateProgramCacheResolution(ProgramOrderCreateDto programOrderCreateDto){
        ...
        if (!Objects.equals(programCacheCreateOrderData.getCode(), BaseCode.SUCCESS.getCode())) {
            throw new TicketStormFrameException(Objects.requireNonNull(BaseCode.getRc(programCacheCreateOrderData.getCode())));
        }
        ...
        for (int i = 0; i < ticketUserIdList.size(); i++) {
            ...
            SeatVo seatVo =
                    Optional.ofNullable(purchaseSeatList.get(i))
                            .orElseThrow(() -> new TicketStormFrameException(BaseCode.SEAT_NOT_EXIST));
    ...
    private String createOrderByRpc(OrderCreateDto orderCreateDto,List<SeatVo> purchaseSeatList){
        ...
        if (!Objects.equals(createOrderResponse.getCode(), BaseCode.SUCCESS.getCode())) {
            ...
            throw new TicketStormFrameException(createOrderResponse);
        }
    ...
    private String createOrderByMq(OrderCreateDto orderCreateDto,List<SeatVo> purchaseSeatList){
        ...
        createOrderSend.sendMessage(JSON.toJSONString(orderCreateDto),sendResult -> {
            ...
            log.error("创建订单失败 需人工处理 orderCreateDto : {}",JSON.toJSONString(orderCreateDto));
            updateProgramCacheDataResolution(orderCreateDto.getProgramId(),purchaseSeatList,OrderStatus.CANCEL);
            createOrderMqDomain.ticketStormFrameException = new TicketStormFrameException(ex);
            latch.countDown();
        });
        try {
            latch.await();
            ...
            throw new TicketStormFrameException(e);
        }
        if (Objects.nonNull(createOrderMqDomain.ticketStormFrameException)) {
            throw createOrderMqDomain.ticketStormFrameException;
        }
