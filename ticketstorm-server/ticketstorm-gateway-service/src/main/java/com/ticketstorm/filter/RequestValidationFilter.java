...
import com.ticketstorm.conf.RequestTemporaryWrapper;
import com.ticketstorm.enums.BaseCode;
import com.ticketstorm.exception.ArgumentError;
import com.ticketstorm.exception.ArgumentException;
import com.ticketstorm.exception.TicketStormFrameException;
import com.ticketstorm.pro.limit.RateLimiter;
import com.ticketstorm.pro.limit.RateLimiterProperty;
import com.ticketstorm.property.GatewayProperty;
import com.ticketstorm.service.ApiRestrictService;
import com.ticketstorm.service.ChannelDataService;
import com.ticketstorm.service.TokenService;
import com.ticketstorm.threadlocal.BaseParameterHolder;
import com.ticketstorm.util.RsaSignTool;
import com.ticketstorm.util.RsaTool;
...
import static com.ticketstorm.constant.Constant.GRAY_PARAMETER;
import static com.ticketstorm.constant.Constant.TRACE_ID;
import static com.ticketstorm.constant.GatewayConstant.BUSINESS_BODY;
import static com.ticketstorm.constant.GatewayConstant.CODE;
import static com.ticketstorm.constant.GatewayConstant.ENCRYPT;
import static com.ticketstorm.constant.GatewayConstant.NO_VERIFY;
import static com.ticketstorm.constant.GatewayConstant.REQUEST_BODY;
import static com.ticketstorm.constant.GatewayConstant.TOKEN;
import static com.ticketstorm.constant.GatewayConstant.USER_ID;
import static com.ticketstorm.constant.GatewayConstant.V2;
import static com.ticketstorm.constant.GatewayConstant.VERIFY_VALUE;
...
public class RequestValidationFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(final ServerWebExchange exchange, final GatewayFilterChain chain) {
        if (rateLimiterProperty.getRateSwitch()) {
            try {
                ...
                throw new TicketStormFrameException(BaseCode.THREAD_INTERRUPTED);
            } finally {
                rateLimiter.release();
            }
        }else{
    ...
    private Map<String,String> doExecute(String originalBody,ServerWebExchange exchange){
        ...
        if ((!allowNormalAccess) && (VERIFY_VALUE.equals(noVerify))) {
            throw new TicketStormFrameException(BaseCode.ONLY_SIGNATURE_ACCESS_IS_ALLOWED);
        }
        if (checkParameter(originalBody,noVerify) && !skipCheckParameter(url)) {
            ...
            if (!checkFlag) {
                throw new TicketStormFrameException(BaseCode.RSA_SIGN_ERROR);
            }
