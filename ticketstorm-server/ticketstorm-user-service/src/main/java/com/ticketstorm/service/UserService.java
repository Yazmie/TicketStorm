...
import com.ticketstorm.entity.TicketUser;
import com.ticketstorm.entity.User;
import com.ticketstorm.entity.UserEmail;
import com.ticketstorm.entity.UserMobile;
...
import com.ticketstorm.mapper.TicketUserMapper;
import com.ticketstorm.mapper.UserEmailMapper;
import com.ticketstorm.mapper.UserMapper;
import com.ticketstorm.mapper.UserMobileMapper;
...
import static com.ticketstorm.core.DistributedLockConstants.REGISTER_USER_LOCK;
...
public class UserService extends ServiceImpl<UserMapper, User> {
    ...
    public void doExist(String mobile){
        ...
        if (contains) {
            ...
            if (Objects.nonNull(userMobile)) {
                throw new TicketStormFrameException(BaseCode.USER_EXIST);
            }
        }
    }
    ...
    public UserLoginVo login(UserLoginDto userLoginDto) {
        ...
        if (StringUtil.isEmpty(mobile) && StringUtil.isEmpty(email)) {
            throw new TicketStormFrameException(BaseCode.USER_MOBILE_AND_EMAIL_NOT_EXIST);
        }
        Long userId;
        if (StringUtil.isNotEmpty(mobile)) {
            ...
            if (StringUtil.isNotEmpty(errorCountStr) && Integer.parseInt(errorCountStr) >= ERROR_COUNT_THRESHOLD) {
                throw new TicketStormFrameException(BaseCode.MOBILE_ERROR_COUNT_TOO_MANY);
            }
            LambdaQueryWrapper<UserMobile> queryWrapper = Wrappers.lambdaQuery(UserMobile.class)
                    .eq(UserMobile::getMobile, mobile);
            UserMobile userMobile = userMobileMapper.selectOne(queryWrapper);
            if (Objects.isNull(userMobile)) {
                ...
                throw new TicketStormFrameException(BaseCode.USER_MOBILE_EMPTY);
            }
            ...
            if (StringUtil.isNotEmpty(errorCountStr) && Integer.parseInt(errorCountStr) >= ERROR_COUNT_THRESHOLD) {
                throw new TicketStormFrameException(BaseCode.EMAIL_ERROR_COUNT_TOO_MANY);
            }
            ...
            if (Objects.isNull(userEmail)) {
                ...
                throw new TicketStormFrameException(BaseCode.USER_EMAIL_NOT_EXIST);
            }
        ...
        if (Objects.isNull(user)) {
            throw new TicketStormFrameException(BaseCode.NAME_PASSWORD_ERROR);
        }
    ...
    private GetChannelDataVo getChannelDataByClient(String code){
        ...
        throw new TicketStormFrameException("没有找到ChannelData");
    }
    ...
    public void authentication(UserAuthenticationDto userAuthenticationDto){
        ...
        if (Objects.equals(user.getRelAuthenticationStatus(), BusinessStatus.YES.getCode())) {
            throw new TicketStormFrameException(BaseCode.USER_AUTHENTICATION);
        }
        User updateUser = new User();
