...
import com.ticketstorm.client.PayClient;
...
import com.ticketstorm.dto.OrderGetDto;
import com.ticketstorm.dto.OrderListDto;
import com.ticketstorm.dto.OrderPayCheckDto;
import com.ticketstorm.dto.OrderPayDto;
import com.ticketstorm.dto.OrderTicketUserCreateDto;
...
import com.ticketstorm.entity.Order;
import com.ticketstorm.entity.OrderTicketUser;
import com.ticketstorm.entity.OrderTicketUserAggregate;
import com.ticketstorm.enums.BaseCode;
import com.ticketstorm.enums.BusinessStatus;
import com.ticketstorm.enums.OrderStatus;
import com.ticketstorm.enums.PayBillStatus;
import com.ticketstorm.enums.PayChannel;
import com.ticketstorm.enums.SellStatus;
import com.ticketstorm.exception.TicketStormFrameException;
import com.ticketstorm.mapper.OrderMapper;
import com.ticketstorm.mapper.OrderTicketUserMapper;
...
import com.ticketstorm.request.CustomizeRequestWrapper;
import com.ticketstorm.service.delaysend.DelayOperateProgramDataSend;
import com.ticketstorm.service.properties.OrderProperties;
...
import com.ticketstorm.vo.OrderGetVo;
import com.ticketstorm.vo.OrderListVo;
import com.ticketstorm.vo.OrderPayCheckVo;
import com.ticketstorm.vo.OrderTicketInfoVo;
import com.ticketstorm.vo.SeatVo;
import com.ticketstorm.vo.TicketUserInfoVo;
import com.ticketstorm.vo.TicketUserVo;
import com.ticketstorm.vo.TradeCheckVo;
import com.ticketstorm.vo.UserAndTicketUserInfoVo;
import com.ticketstorm.vo.UserGetAndTicketUserListVo;
import com.ticketstorm.vo.UserInfoVo;
...
import static com.ticketstorm.core.DistributedLockConstants.UPDATE_ORDER_STATUS_LOCK;
import static com.ticketstorm.core.RepeatExecuteLimitConstants.CANCEL_PROGRAM_ORDER;
import static com.ticketstorm.core.RepeatExecuteLimitConstants.CREATE_PROGRAM_ORDER_MQ;
import static com.ticketstorm.core.RepeatExecuteLimitConstants.PROGRAM_CACHE_REVERSE_MQ;
...
public class OrderService extends ServiceImpl<OrderMapper, Order> {
    ...
    public String create(OrderCreateDto orderCreateDto) {
        ...
        if (Objects.nonNull(oldOrder)) {
            throw new TicketStormFrameException(BaseCode.ORDER_EXIST);
        }
    ...
    public String pay(OrderPayDto orderPayDto) {
        ...
        if (Objects.isNull(order)) {
            throw new TicketStormFrameException(BaseCode.ORDER_NOT_EXIST);
        }
        if (Objects.equals(order.getOrderStatus(), OrderStatus.CANCEL.getCode())) {
            throw new TicketStormFrameException(BaseCode.ORDER_CANCEL);
        }
        if (Objects.equals(order.getOrderStatus(), OrderStatus.PAY.getCode())) {
            throw new TicketStormFrameException(BaseCode.ORDER_PAY);
        }
        if (Objects.equals(order.getOrderStatus(), OrderStatus.REFUND.getCode())) {
            throw new TicketStormFrameException(BaseCode.ORDER_REFUND);
        }
        if (orderPayDto.getPrice().compareTo(order.getOrderPrice()) != 0) {
            throw new TicketStormFrameException(BaseCode.PAY_PRICE_NOT_EQUAL_ORDER_PRICE);
        }
        ...
        if (!Objects.equals(payResponse.getCode(), BaseCode.SUCCESS.getCode())) {
            throw new TicketStormFrameException(payResponse);
        }
    ...
    public OrderPayCheckVo payCheck(OrderPayCheckDto orderPayCheckDto){
        ...
        tradeCheckDto.setChannel(Optional.ofNullable(PayChannel.getRc(orderPayCheckDto.getPayChannelType()))
                .map(PayChannel::getValue).orElseThrow(() -> new TicketStormFrameException(BaseCode.PAY_CHANNEL_NOT_EXIST)));
        ...
        if (!Objects.equals(tradeCheckVoApiResponse.getCode(), BaseCode.SUCCESS.getCode())) {
            throw new TicketStormFrameException(tradeCheckVoApiResponse);
        }
        TradeCheckVo tradeCheckVo = Optional.ofNullable(tradeCheckVoApiResponse.getData())
                .orElseThrow(() -> new TicketStormFrameException(BaseCode.PAY_BILL_NOT_EXIST));
        if (tradeCheckVo.isSuccess()) {
            ...
            throw new TicketStormFrameException(BaseCode.PAY_TRADE_CHECK_ERROR);
        }
        return orderPayCheckVo;
    }
    ...
    public String alipayNotify(HttpServletRequest request){
        ...
        try {
            ...
            if (!Objects.equals(notifyResponse.getCode(), BaseCode.SUCCESS.getCode())) {
                throw new TicketStormFrameException(notifyResponse);
            }
    ...
    public void updateOrderRelatedData(Long orderNumber,OrderStatus orderStatus){
        if (!(Objects.equals(orderStatus.getCode(), OrderStatus.CANCEL.getCode()) ||
            ...
            throw new TicketStormFrameException(BaseCode.OPERATE_ORDER_STATUS_NOT_PERMIT);
        }
        LambdaQueryWrapper<Order> orderLambdaQueryWrapper =
                Wrappers.lambdaQuery(Order.class).eq(Order::getOrderNumber, orderNumber);
        Order order = orderMapper.selectOne(orderLambdaQueryWrapper);
        ...
        if (updateOrderResult <= 0 || updateTicketUserOrderResult <= 0) {
            throw new TicketStormFrameException(BaseCode.ORDER_CANAL_ERROR);
        }
        ...
        if (CollectionUtil.isEmpty(orderTicketUserList)) {
            throw new TicketStormFrameException(BaseCode.TICKET_USER_ORDER_NOT_EXIST);
        }
        if (Objects.equals(orderStatus.getCode(), OrderStatus.CANCEL.getCode())) {
            redisCache.incrBy(RedisKeyBuild.createRedisKey(
    ...
    public void updateProgramRelatedDataResolution(Long programId,Map<Long,List<Long>> seatMap,OrderStatus orderStatus){
        ...
        if (CollectionUtil.isEmpty(seatVoMap)) {
            throw new TicketStormFrameException(BaseCode.LOCK_SEAT_LIST_EMPTY);
        }
    ...
    public OrderGetVo get(OrderGetDto orderGetDto) {
        ...
        if (!Objects.equals(userGetAndTicketUserApiResponse.getCode(), BaseCode.SUCCESS.getCode())) {
            throw new TicketStormFrameException(userGetAndTicketUserApiResponse);
            
        }
        UserGetAndTicketUserListVo userAndTicketUserListVo =
                Optional.ofNullable(userGetAndTicketUserApiResponse.getData())
                        .orElseThrow(() -> new TicketStormFrameException(BaseCode.RPC_RESULT_DATA_EMPTY));
        if (Objects.isNull(userAndTicketUserListVo.getUserVo())) {
            throw new TicketStormFrameException(BaseCode.USER_EMPTY);
        }
    ...
    public boolean initiateCancel(OrderCancelDto orderCancelDto){
        ...
        if (!Objects.equals(order.getOrderStatus(), OrderStatus.NO_PAY.getCode())) {
            throw new TicketStormFrameException(BaseCode.CAN_NOT_CANCEL);
        }
