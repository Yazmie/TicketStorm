...
public class DefaultExceptionHandler {
    ...
    * 业务异常
    * */
    @ExceptionHandler(value = TicketStormFrameException.class)
    public ApiResponse<String> toolkitExceptionHandler(HttpServletRequest request, TicketStormFrameException ticketStormFrameException) {
        log.error("业务异常 method : {} url : {} query : {} ", request.getMethod(), getRequestUrl(request), getRequestQuery(request), ticketStormFrameException);
        return ApiResponse.error(ticketStormFrameException.getCode(), ticketStormFrameException.getMessage());
    }
    /**
     * 参数验证异常
     */
